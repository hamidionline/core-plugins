...
class j06002channelmanagement_framework_import_properties {
	function __construct($componentArgs)
		...
		if (!empty($property_names)) {
			...
			$output['CHANNELMANAGEMENT_FRAMEWORK_PROPERTY_IMPORT_ALL_IMPORTED'] = jr_gettext('CHANNELMANAGEMENT_FRAMEWORK_PROPERTY_IMPORT_ALL_IMPORTED', 'CHANNELMANAGEMENT_FRAMEWORK_PROPERTY_IMPORT_ALL_IMPORTED', false);

			$pageoutput[] = $output;
			$tmpl = new patTemplate();
			$tmpl->addRows( 'pageoutput', $pageoutput );
			$tmpl->setRoot( $ePointFilepath.'templates'.JRDS.find_plugin_template_directory() );
			$tmpl->readTemplatesFromInput( 'channelmanagement_framework_import_properties_all_Imported.html' );
			echo $tmpl->getParsedTemplate();
		}
