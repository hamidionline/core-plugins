		...
		if (!empty($dictionary_array)) {
			foreach ($dictionary_array as $dictionary) {
				if (!file_exists(JOMRES_CHANNEL_DICTIONARIES.JRDS.$this->current_channel_manager.JRDS.$dictionary.'.json')) {
					$response = $this->channelmanagement_rentalsunited_communication->communicate( array() , $dictionary );
					if ($response != false ) {
	...
	*/
	function get_mappable_dictionary_items()
	{
		$arr = array (
			...
			//"Pull_ListDestinations_RQ" =>  array ( "type" => 'Destinations', "sub_type" => "Destination" , "friendly_name" => jr_gettext('_JOMRES_COM_MR_VRCT_PROPERTY_HEADER_REGION','_JOMRES_COM_MR_VRCT_PROPERTY_HEADER_REGION',false) , "jomres_type" => "location" , "id_attribute" => 'DestinationID'  ), 
