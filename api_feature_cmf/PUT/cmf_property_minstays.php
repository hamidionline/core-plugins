...
Flight::route('PUT /cmf/property/minstays', function()
	{
	...
	foreach ( $date_sets as $date_set ) {
		...
		if ( !isset($date_set->minstay) || $date_set->minstay < 1 ) {
			Flight::halt(204, "Minstay not set or less than 1");
			}
			
		$dates_array = array_keys(cmf_utilities::get_date_ranges( $date_set->date_from , $date_set->date_to ));
		foreach ($dates_array as $date ) {
			$epoch = (string)strtotime($date);
			$new_minstay_array[ (string)$epoch] = $date_set->minstay;
		}
	}
	...
	foreach ( $basic_rate_details->multi_query_rates[$property_uid] as $tariff_sets ) {
		...
		$epoch_mindays = $new_minstay_array + $epoch_mindays ;
		ksort($epoch_mindays);
